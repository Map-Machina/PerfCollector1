...
func (p *PerfCollector) sink() {
	...
	for {
		log.Tracef("outer for")
		select {
		case e, ok := <-p.newEncoder:
			if !ok {
				return
			}
			log.Tracef("outer encoder %p", e)
			newEncoder(e)
			continue
		...
		case m, ok := <-measurements:
			...
			for {
				log.Tracef("inner for %p", encoder)
